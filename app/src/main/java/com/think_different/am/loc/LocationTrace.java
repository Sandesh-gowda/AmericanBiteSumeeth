...
import com.think_different.am.util.Constants;
import com.think_different.am.util.MyApplication;
import com.think_different.am.util.OkHttp_Class;
import com.think_different.am.util.User;
...
//      Toast.makeText(context, "Lat"+latitude+"long"+longitude,Toast.LENGTH_SHORT).show();
    ...
    */
    ...
    private void checkValidation() {
      ...
      //  easyPuzzle = MyApplication.getInstance().getPrefManager(Constants.Login_Preferences).getUser().getTripid();
        ...
        if (Controller.getInstance().IsInternetConnected())
            new ContactUsTask(latitude, longitude).execute();
    ...
    private class ContactUsTask extends AsyncTask<Void, Void, JSONObject> {
        private String lan, lat;


        public ContactUsTask(String lan, String lat) {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
           dialog = new ProgressDialog(LocationTrace.this);
        ...
        protected JSONObject doInBackground(Void... params) {
            ...
            try {
                json = new OkHttp_Class().locatintrace(lan, lat);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return json;
           ...
           if (dialog.isShowing())
            ...
            try {
                if (jsonObject != null) {
                    ...
                    String shop_address = jsonObject.getString("address");
                    String distance = jsonObject.getString("distance");
                    MyApplication.getInstance().getPrefManager(Constants.Login_Preferences).storeUser(new User(shop_id, shop_address,distance));
